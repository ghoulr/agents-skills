...
async function iframesStr(cdp, sid, rootFrameId) {
  const { targetInfos } = await cdp.send('Target.getTargets');
  const allIframes = targetInfos.filter(t => t.type === 'iframe');

  function pageDescendants() {
    const visited = new Set([rootFrameId]);
    const queue = [rootFrameId];
    const result = [];
    while (queue.length) {
      const parent = queue.shift();
      for (const t of allIframes) {
        if (t.parentFrameId === parent && !visited.has(t.targetId)) {
  ...
  const pageIframes = pageDescendants();
  const directChildren = pageIframes.filter(t => t.parentFrameId === rootFrameId);
  ...
  if (pageIframes.length === 0) {
    ...
    try {
      ...
      function collect(node, depth = 0) {
        if (depth > 0) {
          frameTreeFrames.push({ id: node.frame.id, url: node.frame.url || '', name: node.frame.name || '', depth });
        }
        for (const child of node.childFrames || []) collect(child, depth + 1);
      }
      collect(frameTree);
    } catch {}
    if (frameTreeFrames.length === 0) return 'No iframes found in this page.';
    const lines = [];
    for (const frame of frameTreeFrames) {
      const match = allIframes.find(t => t.targetId === frame.id);
      const tag = match ? match.targetId.slice(0, 8) : '(same-origin)';
      const indent = '  '.repeat(frame.depth - 1);
      lines.push(`${indent}${tag.padEnd(16)} ${frame.url}`);
    }
    lines.push('');
    lines.push('Use --frame <targetId> to target a cross-origin iframe.');
    return lines.join('\n');
  }

  function collectNested(parentFrameId, depth = 1) {
    const result = [];
    for (const iframe of pageIframes) {
      if (iframe.parentFrameId === parentFrameId && iframe.parentFrameId !== rootFrameId) {
        result.push({ targetId: iframe.targetId, url: iframe.url || '', depth });
        result.push(...collectNested(iframe.targetId, depth + 1));
      }
    }
    return result;
  }
  ...
  for (const child of directChildren) {
    lines.push(`${child.targetId.slice(0, 8).padEnd(8)}  ${child.url}`);
    for (const desc of collectNested(child.targetId)) {
      const indent = '  '.repeat(desc.depth);
      lines.push(`${indent}${desc.targetId.slice(0, 8).padEnd(8)}  ${desc.url}`);
    }
  }
  lines.push('');
  lines.push('Use --frame <targetId> with eval/snap/html/click to target a specific iframe.');
  return lines.join('\n');
}
...
async function runDaemon(targetId) {
  ...
  let rootFrameId = targetId;
  try {
    const { frameTree } = await cdp.send('Page.getFrameTree', {}, sessionId);
    rootFrameId = frameTree.frame.id;
  } catch {}

  async function getPageIframeIds() {
    const { targetInfos } = await cdp.send('Target.getTargets');
    const allIframes = targetInfos.filter(t => t.type === 'iframe');
    const visited = new Set([rootFrameId]);
    const queue = [rootFrameId];
  ...
  async function resolveFrameSid(framePrefix) {
    const pageIds = await getPageIframeIds();
    const resolvedId = resolvePrefix(framePrefix, pageIds, 'iframe target',
      'Run "cdp iframes <target>" to list available iframes.');
    if (iframeSessions.has(resolvedId)) return iframeSessions.get(resolvedId);
    const attachRes = await cdp.send('Target.attachToTarget', { targetId: resolvedId, flatten: true });
    const sid = attachRes.sessionId;
    iframeSessions.set(resolvedId, sid);
    if (attachRes.waitingForDebugger) {
  ...
  cdp.onEvent('Target.detachedFromTarget', (params) => {
    const stale = params.sessionId;
    for (const [key, sid] of iframeSessions) {
      if (sid === stale) iframeSessions.delete(key);
    }
  });
  ...
  // Handle a command
  async function handleCommand({ cmd, args, frame }) {
    resetIdle();
    try {
      let useSid = sessionId;
      if (frame) {
        try {
          useSid = await resolveFrameSid(frame);
        } catch (e) {
          throw new Error(`Cannot attach to frame ${frame}: ${e.message}`);
        }
      }
      let result;
      switch (cmd) {
        ...
        case 'iframes': result = await iframesStr(cdp, useSid, rootFrameId); break;
        case 'snap': case 'snapshot': result = await snapshotStr(cdp, useSid, true); break;
        case 'eval': result = await evalStr(cdp, useSid, args[0]); break;
        case 'shot': case 'screenshot': result = await shotStr(cdp, useSid, args[0], targetId); break;
        case 'html': result = await htmlStr(cdp, useSid, args[0]); break;
        case 'nav': case 'navigate': result = await navStr(cdp, useSid, args[0]); break;
        case 'net': case 'network': result = await netStr(cdp, useSid); break;
        case 'click': result = await clickStr(cdp, useSid, args[0]); break;
        case 'clickxy': result = await clickXyStr(cdp, useSid, args[0], args[1]); break;
        case 'type': result = await typeStr(cdp, useSid, args[0]); break;
        case 'loadall': result = await loadAllStr(cdp, useSid, args[0], args[1] ? parseInt(args[1]) : 1500); break;
        case 'evalraw': result = await evalRawStr(cdp, useSid, args[0], args[1]); break;
...
Usage: cdp <command> [args]
  ...
  iframes <target>                  List iframe targets in the page
  ...
  All commands that take <target> also accept --frame <iframeId> to target
  a cross-origin iframe. Use "cdp iframes <target>" to discover iframe IDs.
...
DAEMON IPC (for advanced use / scripting)
  ...
  Protocol: newline-delimited JSON (one JSON object per line, UTF-8).
    Request:  {"id":<number>, "cmd":"<command>", "args":["arg1","arg2",...]}
              Optional: add "frame":"<iframeTargetId>" to route the command
              into a cross-origin iframe (auto-attaches on first use, then cached).
    Response: {"id":<number>, "ok":true,  "result":"<string>"}
           or {"id":<number>, "ok":false, "error":"<message>"}
  Commands mirror the CLI: snap, eval, shot, html, nav, net, click, clickxy,
  type, loadall, evalraw, iframes, stop. Use evalraw to send arbitrary CDP methods.
  The socket disappears after 20 min of inactivity or when the tab closes.
`;

const NEEDS_TARGET = new Set([
  'snap','snapshot','eval','shot','screenshot','html','nav','navigate',
  'net','network','click','clickxy','type','loadall','evalraw','iframes',
]);
...
async function main() {
  ...
  let frameId = null;
  const frameIdx = cmdArgs.indexOf('--frame');
  if (frameIdx !== -1) {
    if (frameIdx + 1 >= cmdArgs.length) {
      console.error('Error: --frame requires an iframe target ID argument');
      process.exit(1);
    }
    frameId = cmdArgs[frameIdx + 1];
    cmdArgs.splice(frameIdx, 2);
  }
  ...
  const response = await sendCommand(conn, { cmd, args: cmdArgs, frame: frameId });
